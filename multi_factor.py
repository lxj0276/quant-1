...
def select_strategy(context):
    ...
    adjust_condition_config = [
        [True, '_time_c_', '调仓时间', Time_condition, {
            'times': [[14, 48]],  # 调仓时间列表，二维数组，可指定多个时间点
        }],
    ...
    pick_config = [
        ...
        [True, '', '多因子选股票池', Pick_financial_data, {
            'factors': [
                FD_Factor('valuation.market_cap', min=0, max=100)  # 流通市值0~100亿
                , FD_Factor('valuation.pe_ratio', min=0, max=200)  # pe > 0
                , FD_Factor('indicator.eps', min=0)  # eps > 0
                , FD_Factor('indicator.roe',min=0.1) # roe
                , FD_Factor('indicator.inc_net_profit_year_on_year',min=3) # inc_revenue_year_on_year
                , FD_Factor('indicator.inc_total_revenue_year_on_year',min=3) # inc_net_profit_year_on_year 
            ],
            'order_by': 'valuation.market_cap',  # 按流通市值排序
            'sort': SortType.asc,  # 从小到大排序
            'limit': 200  # 只取前200只
        }],
        [True, '', '过滤创业板', Filter_gem, {}],
        [False, '', '过滤解禁股', Filter_jj, {}],
        [True, '', '过滤ST,停牌,涨跌停股票', Filter_common, {}],
        [True, '', '权重排序', SortRules, {
            'config': [
                [True, '', '市值排序', Sort_financial_data, {
                ...
                [True, '', '流通市值排序', Sort_financial_data, {
                    ...
                    'sort': SortType.asc
                    , 'weight': 100}],
                [False, '', '市盈率排序', Sort_financial_data, {
                    'factor': 'valuation.pe_ratio',
                    'sort': SortType.asc
                    , 'weight': 100}],
                [True, '', '市净率排序', Sort_financial_data, {
                ...
                [False, '', '按当前价排序', Sort_price, {
                    'sort': SortType.asc
                    , 'weight': 50}],
                [False, '', '按成交量排序', Sort_volume, {
                    'sort': SortType.asc
                    , 'weight': 10
                    , 'day': 20}],
                [False, '', '60日成交额排序', Sort_money, {
                    'sort': SortType.asc
                    , 'weight': 10
                    , 'day': 60}],
                [False, '', '20日成交额排序', Sort_money, {
                    'sort': SortType.asc
                    , 'weight': 10
                    , 'day': 20}],
...
class Filter_jj(Filter_stock_list):
    def filter(self, context, data, stock_list):
        ...
        return [stock for stock in stock_list if stock not in df_jj.code.values]
...
# 按N日成交量排序
# day 指定几日累计成交量,默认为5
class Sort_volume(SortBase):
    def sort(self, context, data, stock_list):
        day = self._params.get('day', 5)
        r = []
        for stock in stock_list:
            vol = attribute_history(stock, day, '1d', 'volume', True)['volume'].sum()
            r.append([stock, vol])
        r = sorted(r, key=lambda x: x[1], reverse=not self.is_asc)
        return [stock for stock, vol in r]

    def __str__(self):
        return '[权重: %s ] [排序: %s ] 按 %d 日涨幅排序' % (self.weight, self._sort_type_str(), self._params.get('day', 5))

# 按N日成交额排序
# day 指定几日累计成交量,默认为5
class Sort_money(SortBase):
    def sort(self, context, data, stock_list):
        day = self._params.get('day', 5)
        r = []
        for stock in stock_list:
            money = attribute_history(stock, day, '1d', 'money', True)['money'].sum()
            r.append([stock, money])
        r = sorted(r, key=lambda x: x[1], reverse=not self.is_asc)
        return [stock for stock, money in r]
...
class Sort_turnover_ratio(SortBase):
    def sort(self, context, data, stock_list):
        ...
        trade_days = jqdata.get_trade_days(end_date=context.previous_date, count = 20)
        ...
        if self.is_asc:
            turnover_ratio_20_df = turnover_ratio_20_df.sort(columns='turnover_ratio_20', ascending=True)
        else:
            turnover_ratio_20_df = turnover_ratio_20_df.sort(columns='turnover_ratio_20', ascending=False)
        stock_list = list(turnover_ratio_20_df.index)
        return stock_list
